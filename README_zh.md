...
<p style="font-size: 15px; line-height: 1.8; color: #333; margin: 15px 0;">
平台由 <strong>WEB、APP、DEVICE、NODE、VIDEO、AI、TASK、EDGE</strong> 八个核心模块组成，以 Java 构建稳定管控底座、Python 承载 AI 与网络能力、C++ 驱动高性能计算任务，三语言混编各取所长。在能力侧，平台覆盖 GB28181 / ONVIF 多协议摄像头接入、实时与抓拍算法任务、YOLO 目标检测与 SAM 零样本自动标注、人脸/车牌识别、可编排业务后处理、联邦算力集群调度，以及 <strong>无限联邦边缘集群模式</strong>（内存占用约 512MB，Ceph 边缘 0 硬盘占用，一行命令把普通开发板直接智能化，算力可铺开部署并汇聚上云），还有 MQTT / TCP / HTTP 物联网设备全生命周期管理；在体验侧，Web 管控台与移动 App / 小程序能力对齐，让指挥中心与现场巡检同一套业务逻辑、随时随地处置。
</p>
...
<div style="margin: 20px 0; padding: 18px 22px; border-radius: 10px; border: 1px solid rgba(52, 152, 219, 0.25); background: linear-gradient(120deg, #f0f7ff 0%, #ffffff 55%, #eef9f4 100%);">
  ...
  <p style="font-size: 14px; line-height: 1.8; color: #333; margin: 0;">
    内存占用约 <strong>512MB</strong>，<strong>Ceph 边缘 0 硬盘占用</strong>（告警图与业务对象写共享 Ceph，边缘不落本地业务盘）；可直接铺开算力部署；<strong>一行命令</strong>即可把普通开发板直接智能化，同时告警与事件汇聚上云。
    与上方三档全栈部署互补——全栈负责云边管控与业务编排，EDGE 节点负责现场轻量推理与无限横向扩容，真正实现「中心一处管控、边缘任意铺开」。
  </p>
</div>
...
<ul style="font-size: 14px; line-height: 1.8; color: #444; margin: 10px 0;">
  ...
  <li><strong>无限联邦边缘集群模式（EDGE）</strong>：面向 RK3588、树莓派等普通开发板与现场算力节点，提供内存占用约 <strong>512MB</strong> 的轻量无界面边缘算法运行时——<strong>一行命令</strong>写入控制面地址并启动，即可把普通开发板直接智能化，算力可随点位铺开部署，同时告警与事件汇聚上云；告警图与抓拍写入共享 Ceph，由中心 sink 归档，边缘侧<strong>业务存储 0 硬盘占用</strong>（不落本地业务盘、无 MinIO 直传）。执行面从 VIDEO 控制面抽离为独立 <code>EDGE</code> 模块：无 WEB、无本地业务库，专注「收指令、跑推理、回事件」。现场只需配置一项 <code>EDGE_NODE_URL</code>，经 enroll / runtime-config 自动领取 EMQX 集群地址、MQTT 凭证、Ceph 热缓冲路径与 Topic 约定；任意数量节点共享同一 EMQX 总线，由控制面按能力调度实时 / 抓拍 / 巡检任务，启停经 <code>mqtt/iot-algo-task-cmd</code> 下发，心跳、确认、告警与后处理原路回传，真正做到「512MB 起跑、Ceph 边缘 0 硬盘、一行命令上线、无限联邦扩容、现场智能与云端汇聚一体」</li>
...
<ul style="font-size: 14px; line-height: 1.8; margin: 10px 0; padding-left: 20px;">
  <li>无限联邦边缘集群：约 512MB 内存即可接入</li>
  <li>Ceph 边缘 0 硬盘占用，业务对象不落本地盘</li>
  <li>一行命令把 RK3588 等普通开发板直接智能化</li>
  <li>算力随点位铺开部署，告警与事件汇聚上云</li>
</ul>
</div>
...
<td style="padding: 15px; border: 1px solid #e0e0e0; line-height: 1.8; color: #444;">
  <ul style="margin: 5px 0; padding-left: 20px;">
    <li><strong>无限联邦边缘集群模式</strong>：第八核心模块，内存占用约 512MB；Ceph 边缘 0 硬盘占用；一行命令把普通开发板直接智能化，算力可铺开部署并汇聚上云，支撑无限联邦扩容</li>
    <li><strong>无界面边缘算法运行时</strong>：从 VIDEO 抽离的算法执行面；纯 CLI / systemd，不提供 WEB 与本地业务库，专注边缘侧推理与事件回传</li>
    <li><strong>单配置入口</strong>：现场只需配置 <code>EDGE_NODE_URL</code>（iot-node 控制面地址）；经 <code>/admin-api/node/edge/enroll</code> 与 <code>runtime-config</code> 自动领取 MQTT broker 列表、租户凭证、clientId、Ceph 热缓冲路径与算法 Topic 约定</li>
    <li><strong>全链路 MQTT 总线</strong>：订阅 <code>mqtt/iot-algo-task-cmd</code> 接收启停指令（按 <code>targetNodeId</code> 过滤本节点），发布心跳 / ack / 告警 / 后处理；不直连 Kafka、不提供 HTTP 管理面</li>
    <li><strong>Ceph 边缘 0 硬盘占用</strong>：告警图与抓拍写入 Ceph 共享路径，由中心 <code>iot-sink</code> 归档至 MinIO；边缘侧不落本地业务盘、无 MinIO 同步上传职责</li>
    <li><strong>无限集群扩展</strong>：任意数量 EDGE 节点挂载同一 EMQX 集群；broker 列表有序探测、故障从头重试，支撑实时 / 抓拍 / 巡检算法任务横向扩容</li>
    <li><strong>与控制面分工清晰</strong>：VIDEO 保留任务 CRUD、调度策略与日志查询；EDGE 承接 <code>runtime/</code> 推理主循环与 workload 拉起，可与 NODE Agent（通用 HTTP 工作负载）并存</li>
  </ul>
</td>
</tr>
</table>
